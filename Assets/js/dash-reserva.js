...
// =========================
const WORKDAY_START_HOUR = 8;    // 08:00
const WORKDAY_END_HOUR = 21;     // 21:00
const HOURS_PER_DAY = WORKDAY_END_HOUR - WORKDAY_START_HOUR;

const SUBUTIL_THRESHOLD = 0.4;   // < 40% ocupación = subutilizado
const CANCELLED_STATE = "Rechazada"; // Ajusta si tu BD usa otro valor
...
async function fetchReservasRango(fechaInicio, fechaTermino) {
  const { data, error } = await supabase
    .from("reserva")
    .select(`
      id_reserva,
...
async function fetchReservasRangoAnterior(fechaInicio, fechaTermino) {
  ...
  return fetchReservasRango(formatDateForInput(startPrev), formatDateForInput(endPrev));
}
...
async function calcularTasaOcupacion(reservas, fechaInicio, fechaTermino) {
  ...
  // ✅ CORREGIDO: Excluir reservas canceladas del cálculo de ocupación
...
async function calcularCrecimientoReservas(reservasActual, fechaInicio, fechaTermino) {
  const prev = await fetchReservasRangoAnterior(fechaInicio, fechaTermino);
...
function calcularTasaCancelacion(reservas) {
  ...
  // ✅ CORREGIDO: El campo es "estado" no "estado_reserva"
...
async function calcularRankingEspacios(reservas) {
  ...
  // ✅ MEJORADO: Excluir canceladas del ranking
  const reservasValidas = reservas.filter(r => r.estado !== CANCELLED_STATE);
...
async function calcularSubutilizados(reservas, fechaInicio, fechaTermino) {
  const espacios = await fetchEspacios();
  const dias = diffDays(fechaInicio, fechaTermino);
  const horasDisp = dias * HOURS_PER_DAY;

  // ✅ MEJORADO: Excluir canceladas del cálculo de subutilización
  const reservasValidas = reservas.filter(r => r.estado !== CANCELLED_STATE);
...
function descargarReporteCSV() {
  ...
  const { inicio, termino, ocup, crec, canc, ranking, subutil, totalReservas } = ultimoReporte;

  // Construir el CSV con estructura clara
  ...
  // Sección: Espacios Subutilizados
...
async function actualizarDashboard() {
  ...
  const reservas = await fetchReservasRango(inicio, termino);

  const [ocup, crec, canc, ranking, subutil] = await Promise.all([
    calcularTasaOcupacion(reservas, inicio, termino),
    calcularCrecimientoReservas(reservas, inicio, termino),
    calcularTasaCancelacion(reservas),
    calcularRankingEspacios(reservas),
    calcularSubutilizados(reservas, inicio, termino)
  ]);
  ...
  // ✅ Guardar datos para el reporte CSV
  ultimoReporte = {
    inicio,
    termino,
    ocup,
